...
from ego_r1.llm_agent.generation import LLMGenerationManager, GenerationConfig
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1):
    ...
    if adv_estimator == 'gae':
        ...
        token_level_rewards = data.batch['token_level_rewards']
        index = data.non_tensor_batch['uid'] # index = batch.non_tensor_batch['uid']
        responses = data.batch['responses'] # responses = batch.batch['responses']
        response_length = responses.size(-1)
        attention_mask = data.batch['attention_mask'] # attention_mask = batch.batch['attention_mask']
        response_mask = attention_mask[:, -response_length:]
...
class RayPPOTrainer(object):
    ...
    def _create_dataloader(self):
        ...
        # TODO: we have to make sure the batch size is divisible by the dp size next(iter(self.train_dataloader))
    ...
    def _validate(self):
        ...
        if not self.config.do_search:
            ...
            for batch_dict in self.val_dataloader:
                ...
                test_gen_batch.non_tensor_batch['identity'] = test_batch.non_tensor_batch['identity']
                test_gen_batch.meta_info = {
                    'eos_token_id': self.tokenizer.eos_token_id,
                    'pad_token_id': self.tokenizer.pad_token_id,
                    'recompute_log_prob': False,
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                gen_batch.non_tensor_batch['identity'] = batch.non_tensor_batch['identity']
                print(f'Original batch: {batch.non_tensor_batch}')
                # import pdb; pdb.set_trace()
                ####################
                # original code here
                ...
                ####################
                # with _timer('step', timing_raw):
                    else: # self.tokenizer.decode(first_input_ids)
                        ...
                        # (final_gen_batch_output.batch['info_mask'][0] == 0).nonzero().squeeze()
                        # self.tokenizer.decode(final_gen_batch_output.batch['input_ids'][0])
