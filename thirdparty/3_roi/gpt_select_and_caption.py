...
class OpenAIClient:
    ...
    def chat_json(self, model: str, system_prompt: str, user_prompt: str, images: list[Path] | None = None, timeout: int = 120) -> dict:
...
def main():
    parser = argparse.ArgumentParser(description="Select figures and generate rubric + captions in one pass (GPT)")
    ...
    client = OpenAIClient()
    sel_obj = client.chat_json(args.model, system_prompt_sel, user_prompt_sel, images=None, timeout=args.timeout)
    ...
    for fig in figures[: args.max_figures]:
        ...
        obj = client.chat_json(args.model, system_prompt_cap, user_prompt_cap, images=image_paths, timeout=args.timeout)
        outputs.append({
            "figure": str(fig),
...
# python3 thirdparty/3_roi/gpt_select_and_caption.py \
