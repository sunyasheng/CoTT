...
while IFS= read -r local_pdf; do
    if [[ -n "$local_pdf" ]]; then
        ...
        if (( (++BATCH_COUNT) % BATCH_SIZE == 0 )); then
            ...
            bash -c "source ~/.bashrc && conda activate gsam && python3 thirdparty/2_pdf_parsing/batch_infer.py --root ${SHARD_TEMP_DIR}/pdf --outdir ${SHARD_TEMP_DIR}/md --start 1 --end 999999"
